...
# Standalone note: this file is a near-verbatim copy of omicverse's
# ``omicverse/single/_monocle.py`` (the canonical implementation). The
# only diffs from upstream are to drop the omicverse-specific
# ``@register_function`` decorator and swap the lazy
# ``from ..external import monocle2_py as _m2`` for a local backend
# import — everything else is kept in sync so upstream fixes flow in
# via ``git diff``.
...
class Monocle:
    ...
    def __init__(self, adata: AnnData):
        ...
        Notes
        -----
        In the omicverse port this import is lazy to satisfy the
        architecture test; here we just import the backend directly
        since this package *is* the backend.
        """
        from . import (
            core, dimension_reduction, ordering, differential,
        ...
        class _Backend:
            pass
        _m2 = _Backend()
        for _mod in (core, dimension_reduction, ordering, differential,
                     plotting, clustering, utils):
            for _name in dir(_mod):
                if not _name.startswith("_"):
                    setattr(_m2, _name, getattr(_mod, _name))
        self._m2 = _m2
    ...
    def cluster_genes(expression_matrix, k: int, method: str = 'correlation'):
        ...
        """
        from .clustering import cluster_genes as _cluster_genes
        return _cluster_genes(expression_matrix, k, method=method)
